...
test('checkout schema: cart table and unique index', async () => {
  ...
  assert.equal(result.checks.sales_series_x_report_printed_at, true, 'sales_series.x_report_printed_at must exist');
  assert.equal(result.checks.sales_series_x_report_printed_by, true, 'sales_series.x_report_printed_by must exist');
  assert.equal(result.checks.sales_series_z_report_printed_at, true, 'sales_series.z_report_printed_at must exist');
  assert.equal(result.checks.sales_series_z_report_printed_by, true, 'sales_series.z_report_printed_by must exist');
...
test('checkout E2E: new transaction resets X/Z print requirements for close', { skip: process.env.CHECKOUT_E2E !== '1' && 'Set CHECKOUT_E2E=1' }, async () => {
  const username = process.env.CHECKOUT_E2E_USER;
  const password = process.env.CHECKOUT_E2E_PASS;
  const machineName = process.env.CHECKOUT_E2E_MACHINE;
  const barcode = process.env.CHECKOUT_E2E_BARCODE;
  ...
  const [seriesRows] = await getPool().query(
    `SELECT full_series_no FROM sales_series
     WHERE machine_id = ? AND COALESCE(lockbatch, 'N') = 'N'
     ORDER BY ID DESC LIMIT 1`,
    [machineName],
  );
  assert.ok(seriesRows.length, 'open sales series required');
  ...
  const markX = await fetchJson(`${apiBase}/api/pos/series/${encodeURIComponent(seriesNo)}/reports/printed`, {
    method: 'POST',
    headers: authHeaders,
    body: JSON.stringify({ machine_name: machineName, report_type: 'X' }),
  });
  assert.equal(markX.response.status, 200, markX.body.error || 'mark X printed failed');

  const markZ = await fetchJson(`${apiBase}/api/pos/series/${encodeURIComponent(seriesNo)}/reports/printed`, {
    method: 'POST',
    headers: authHeaders,
    body: JSON.stringify({ machine_name: machineName, report_type: 'Z' }),
  });
  assert.equal(markZ.response.status, 200, markZ.body.error || 'mark Z printed failed');

  const beforeCheckoutReq = await fetchJson(
    `${apiBase}/api/pos/series/${encodeURIComponent(seriesNo)}/close-requirements?machine_name=${encodeURIComponent(machineName)}`,
    { headers: authHeaders },
  );
  assert.equal(beforeCheckoutReq.response.status, 200);
  assert.equal(beforeCheckoutReq.body.data.can_close, true, 'series should be closeable after X and Z print marks');
  ...
  const afterCheckoutReq = await fetchJson(
    `${apiBase}/api/pos/series/${encodeURIComponent(seriesNo)}/close-requirements?machine_name=${encodeURIComponent(machineName)}`,
    { headers: authHeaders },
  );
  assert.equal(afterCheckoutReq.response.status, 200);
  assert.equal(afterCheckoutReq.body.data.can_close, false, 'series should not be closeable after a new transaction');
  assert.ok(afterCheckoutReq.body.data.missing_reports.includes('X'), 'X report should be required again');
  assert.ok(afterCheckoutReq.body.data.missing_reports.includes('Z'), 'Z report should be required again');

  const closeAttempt = await fetchJson(`${apiBase}/api/pos/series/${encodeURIComponent(seriesNo)}/close`, {
    method: 'POST',
    headers: authHeaders,
    body: JSON.stringify({ machine_name: machineName }),
  });
  assert.equal(closeAttempt.response.status, 409, 'close should be blocked until X and Z are reprinted');
});
