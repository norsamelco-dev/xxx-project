...
import { syncTerminalConfigToLocal } from '../services/config/terminalConfig'
...
export default function BootstrapScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    async function bootstrap() {
      ...
      let config = saved
      try {
        config = await syncTerminalConfigToLocal(saved)
      } catch {
        // Use saved local config if the server is unreachable during bootstrap.
      }
